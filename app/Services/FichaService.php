...
class FichaService
{
     ...
     * Si las relaciones ya están cargadas en memoria las usa directamente
     * para evitar queries redundantes (patrón N+1).
     * 
     * @param Ficha $ficha
     * @return float
     */
    ...
    {
        if (
            $ficha->relationLoaded('productos') &&
            $ficha->relationLoaded('servicios') &&
            $ficha->relationLoaded('gastos')
        ) {
            return (float) $ficha->productos->sum('precio')
                + (float) $ficha->servicios->sum('precio')
                + (float) $ficha->gastos->sum('precio');
        }
